...
use MusicianBundle\Entity\MusicianFriendship;
...
class MusicianRepository extends EntityRepository
{
    ...
    public function countSongs(Musician $musician)
    {
        return $this->getEntityManager()
            ->createQuery('SELECT COUNT(s) FROM SongBookBundle:Song s WHERE s.owner = :musician')
    ...
    {
        return $this->getEntityManager()
            ->createQuery('SELECT COUNT(g) FROM InstrumentBundle:Guitar g WHERE g.owner = :musician')
            ->setParameter('musician', $musician)
            ->getSingleScalarResult()
        ;
    }
