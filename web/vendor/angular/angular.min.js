...
(function(){'use strict';angular.module('ui.scrollable',[]).directive('scrollable',ScrollableDirective);function ScrollableDirective($document){var options={scrollInertia:100,scrollButtons:{enable:false},theme:'dark-3',scrollAmount:80,axis:'y',contentTouchScroll:true,autoHideScrollbar:false};return{restrict:'EA',replace:true,transclude:true,template:'<div class="scrollable" data-ng-transclude=""></div>',scope:{options:'=options'},link:function(scope,element){if(scope.options){angular.extend(options,scope.options);}
var $element=$(element);initScrollbar();function initScrollbar(){$element.ready(function(){$element.mCustomScrollbar(options);});$(window).on('resize',function(){$element.mCustomScrollbar('update');});$document.on('hover',$element,function(){$document.data({"keyboard-input":"enabled"});$(this).addClass("keyboard-input");});$document.on('mouseout',$element,function(){$document.data({"keyboard-input":"disabled"});$(this).removeClass("keyboard-input");});$document.on('keydown',function(){if($(this).data("keyboard-input")==="enabled"){var activeElem=$(".keyboard-input");var top=parseFloat($(".keyboard-input .mCSB_container").position().top);var activeElemPos=Math.abs(top);var pixelsToScroll=80;if(e.which===38){e.preventDefault();if(pixelsToScroll>activeElemPos){activeElem.mCustomScrollbar("scrollTo","top");}
else{activeElem.mCustomScrollbar("scrollTo",(activeElemPos-pixelsToScroll),{scrollInertia:400,scrollEasing:"easeOutCirc"});}}
else if(e.which===40){e.preventDefault();activeElem.mCustomScrollbar("scrollTo",(activeElemPos+pixelsToScroll),{scrollInertia:400,scrollEasing:"easeOutCirc"});}}});}}};}})();
(function(){'use strict';angular.module('Note',[]);})();
(function(){'use strict';var NoteService=function NoteService($http,$q){this.services['$http']=$http;this.services['$q']=$q;};NoteService.prototype.constructor=NoteService;NoteService.prototype.services={};NoteService.prototype.displayFlat=false;NoteService.prototype.notes=[];NoteService.prototype.current=null;NoteService.prototype.getCurrent=function getCurrent(){return this.current;};NoteService.prototype.setCurrent=function setCurrent(note){this.current=note;};NoteService.prototype.setDisplayFlat=function setDisplayFlat(){};NoteService.prototype.all=function all(){if(0!==this.notes.length){return this.notes}else{return this.find();}};NoteService.prototype.find=function find(){var deferred=this.services.$q.defer();this.services.$http.get(Routing.generate('theory_note',{_format:'json'}),{}).success(function(response){this.notes=response;this.apply(function(note){if(this.displayFlat){}}.bind(this));deferred.resolve(response);}.bind(this)).error(function(response){deferred.reject(response);});return deferred.promise;};NoteService.prototype.get=function get(value){return this.notes.find(function findByValue(element){return value==element.value;});};NoteService.prototype.addSemitone=function addSemitone(reference,semitones){var newValue=(reference.value+semitones)%12;return this.get(newValue);};NoteService.prototype.apply=function apply(callback){if(typeof callback==='function'){for(var i=0;i<this.notes.length;i++){callback(this.notes[i]);}}};angular.module('Note').service('NoteService',['$http','$q',NoteService]);})();
(function(){'use strict';angular.module('Note').directive('noteList',['NoteService',function(NoteService){return{restrict:'E',templateUrl:assetDirectory+'/musictoolstheory/js/Note/Partial/list.html',replace:true,scope:{},link:function(scope,element,attrs){NoteService.all().then(function(data){scope.notes=data;});}};}]);})();
(function(){'use strict';angular.module('Note').directive('noteSelector',['NoteService',function NoteSelectorService(NoteService){return{restrict:'E',templateUrl:assetDirectory+'/musictoolstheory/js/Note/Partial/selector.html',replace:true,scope:{},controller:function NoteSelectorController(){},controllerAs:'noteSelectorCtrl',bindToController:true,link:function(scope,element,attrs){NoteService.all().then(function(data){scope.notes=data;});}};}]);})();
(function(){'use strict';angular.module('Guitar',[]);})();
(function(){'use strict';angular.module('Guitar').directive('guitarMenu',[function(){return{restrict:'E',templateUrl:assetDirectory+'/musictoolsinstrument/js/Guitar/Partial/guitar-menu.html',replace:true,scope:{},link:function(scope,element,attrs){}};}]);})();
(function(){'use strict';angular.module('GuitarNeck',['Guitar']);})();
(function(){'use strict';angular.module('GuitarNeck',[]).directive('guitarNeckWidget',[function(){var strings=[{value:7},{value:2},{value:10},{value:5},{value:0},{value:7}];return{restrict:'E',templateUrl:assetDirectory+'/musictoolsinstrument/js/GuitarNeck/Partial/guitar-neck-widget.html',replace:true,scope:{},link:function(scope,element,attrs){var canvas=element.find('canvas').get(0);if(angular.isDefined(canvas)&&angular.isDefined(canvas)){var neck=new GuitarNeck(canvas,element.width(),strings);neck.render();}}};}]);var GuitarNeck=function GuitarNeckConstructor(canvas,width,strings){this.canvas=canvas;this.context=canvas.getContext('2d');this.strings=strings;this.width=width;this.notes=new Notes();};GuitarNeck.prototype={constructor:GuitarNeck,canvas:null,context:null,notes:null,width:1000,height:{strings:{container:280,offset:20,interval:null},references:{container:20},calculate:function calculateHeight(){return this.strings.container+this.references.container;}},strings:{},frets:{offset:60,start:0,end:24,showNumber:true,highlightReferences:true,interval:null},renderOptions:{defaultColor:'#161616',defaultFill:'#777777',stringColor:'#AAAAAA',fretColor:'#303030',fretHighlightColor:'#444444',dotSize:10,noteSize:30,fretNumberSize:25,notes:{size:30,defaultColor:'#161616',defaultFill:'#AAAAAA',namingRule:'sharp',alteration:{display:true,size:25}}},render:function(){if(null!==this.context){this.canvas.width=this.width;var stringsHeight=this.height.strings.container-this.height.strings.offset*2;if(this.frets.showNumber){stringsHeight-=this.renderOptions.fretNumberSize;}
this.height.strings.interval=stringsHeight/(this.strings.length-1);this.frets.interval=(this.width-this.frets.offset-20)/(this.frets.end-this.frets.start);this.drawFrets();this.drawStrings();this.drawNotes();}},drawFrets:function(){var fretPosition=this.frets.offset;for(var j=this.frets.start;j<=this.frets.end;j++){var highlight=false;if(0!==j&&(j%12)==0){this.drawFretReference(fretPosition-(this.frets.interval/2),true);highlight=true;}else{var caseNumber=j%12;if(13!==j&&caseNumber<10&&(caseNumber%2)!=0){this.drawFretReference(fretPosition-(this.frets.interval/2),false);highlight=true;}}
this.drawFret(fretPosition,j,highlight);fretPosition+=this.frets.interval;}},drawFret:function(x,number,highlight){this.context.beginPath();var lineWidth=3;if(this.frets.highlightReferences){if(0==number){lineWidth=9;}else if(highlight){lineWidth=5;}}
var fretColor=this.renderOptions.fretColor;if(this.frets.highlightReferences&&highlight){fretColor=this.renderOptions.fretHighlightColor;}
if(0===(x/0.5)%2){x+=0.5;}
this.context.moveTo(x,0);this.context.lineTo(x,this.height.strings.container);this.context.lineWidth=lineWidth;this.context.strokeStyle=fretColor;this.context.closePath();this.context.stroke();if(this.frets.showNumber){var y=0;this.context.beginPath();this.context.fillStyle=fretColor;this.context.rect(x-(this.renderOptions.fretNumberSize/2),y,this.renderOptions.fretNumberSize,this.renderOptions.fretNumberSize);this.context.closePath();this.context.fill();this.context.beginPath();this.context.font='bold 11pt Calibri';this.context.fillStyle=this.renderOptions.defaultColor;this.context.textAlign='center';this.context.shadowColor='transparent';this.context.closePath();this.context.fillText(number,x,y+17);}},drawFretReference:function(x,double){var w=this.renderOptions.dotSize;this.context.beginPath();this.context.fillStyle=this.renderOptions.defaultFill;var y=this.height.strings.container+w;if(double){this.context.arc(x+w,y,w/2,0,2*Math.PI,false);this.context.arc(x-w,y,w/2,0,2*Math.PI,false);}else{this.context.arc(x,y,w/2,0,2*Math.PI,false);}
this.context.closePath();this.context.fill();},drawStrings:function(){for(var i=this.strings.length-1;i>=0;i--){this.drawString(i,this.strings[i].value);}},drawString:function(stringNumber,stringValue){var note=this.notes.get(stringValue);var stringPosition=this.getStringPosition(stringNumber);var stringWidth=((stringNumber+1)*0.5)+0.5;this.context.beginPath();this.context.moveTo(0,stringPosition);this.context.lineTo(this.width,stringPosition);this.context.lineWidth=stringWidth;this.context.strokeStyle=this.renderOptions.stringColor;this.context.shadowColor='#000';this.context.shadowBlur=4;this.context.shadowOffsetX=0;this.context.shadowOffsetY=0;this.context.closePath();this.context.stroke();this.drawNote((this.renderOptions.notes.size+2)/2,stringPosition,note,this.renderOptions.notes.size,this.renderOptions.notes.defaultColor,this.renderOptions.notes.defaultFill,'#dddddd');},getStringPosition:function(stringNumber){var stringPosition=this.height.strings.interval*stringNumber+0.5+this.height.strings.offset;if(this.frets.showNumber){stringPosition+=this.renderOptions.fretNumberSize;}
return stringPosition;},drawNotes:function(){for(var i=this.strings.length-1;i>=0;i--){var stringPosition=this.getStringPosition(i);var note=this.notes.get(this.strings[i].value);var start=this.frets.start;if(this.frets.start==0){start+=1;}
for(var j=start;j<=this.frets.end;j++){var nextNote=this.notes.addSemitone(note,j);if(!nextNote.altered||this.renderOptions.notes.alteration.display){var fretPosition=this.frets.interval*j+this.frets.offset;this.drawNote(fretPosition-(this.frets.interval/2),stringPosition,nextNote);}}}},drawNote:function(x,y,note,size,color,fill,border){var noteSize=size||(note.altered?this.renderOptions.notes.alteration.size:this.renderOptions.notes.size);var noteFill=fill||note.render.fill;var noteColor=color||note.render.color;var noteName=null;switch(this.renderOptions.notes.namingRule){case'both':noteName=note.name.flat!=note.name.sharp?note.name.sharp+' / '+note.name.flat:note.name.flat;break;case'flat':noteName=note.name.flat;break;case'sharp':noteName=note.name.sharp;break;}
this.context.beginPath();this.context.fillStyle=noteFill;this.context.shadowColor='#000';this.context.shadowBlur=4;this.context.shadowOffsetX=0;this.context.shadowOffsetY=0;this.context.arc(x,y,noteSize/2,0,2*Math.PI,false);this.context.closePath();this.context.fill();if(border){this.context.lineWidth=1;this.context.strokeStyle=border;this.context.shadowColor='transparent';this.context.stroke();}
this.context.beginPath();this.context.font='bold 12pt Calibri';this.context.fillStyle=noteColor;this.context.textAlign='center';this.context.shadowColor='transparent';this.context.closePath();this.context.fillText(noteName,x,y+5);}};var Notes=function NotesConstructor(){};Notes.prototype={constructor:Notes,notes:[{name:{sharp:'A',flat:'A'},value:0,render:{color:'#ffffff',fill:'#8f0000'}},{name:{sharp:'A♯',flat:'B♭'},value:1,render:{color:'#ffffff',fill:'#a23e97'},altered:true},{name:{sharp:'B',flat:'B'},value:2,render:{color:'#ffffff',fill:'#6a489d'}},{name:{sharp:'C',flat:'C'},value:3,render:{color:'#ffffff',fill:'#006cb7'}},{name:{sharp:'C♯',flat:'D♭'},value:4,render:{color:'#ffffff',fill:'#008e83'},altered:true},{name:{sharp:'D',flat:'D'},value:5,render:{color:'#ffffff',fill:'#00854a'}},{name:{sharp:'D♯',flat:'E♭'},value:6,render:{color:'#ffffff',fill:'#7fb439'},altered:true},{name:{sharp:'E',flat:'E'},value:7,render:{color:'#ffffff',fill:'#fdb813'}},{name:{sharp:'F',flat:'F'},value:8,render:{color:'#ffffff',fill:'#584742'}},{name:{sharp:'F♯',flat:'G♭'},value:9,render:{color:'#ffffff',fill:'#c15e20'},altered:true},{name:{sharp:'G',flat:'G'},value:10,render:{color:'#ffffff',fill:'#f58220'}},{name:{sharp:'G♯',flat:'A♭'},value:11,render:{color:'#ffffff',fill:'#f04e46'},altered:true}],get:function(value){return this.notes.find(function findByName(element){return value==element.value;});},addSemitone:function(reference,semitones){var newValue=(reference.value+semitones)%12;return this.get(newValue);}};})();
(function(){'use strict';angular.module('SheetMusic',[]);})();
(function(){'use strict';var SheetMusicController=function SheetMusicController(){};SheetMusicController.prototype.file=null;SheetMusicController.prototype.component={};SheetMusicController.prototype.player={ready:false,state:0,component:{}};SheetMusicController.prototype.play=function play(){this.player.component.Play();};SheetMusicController.prototype.pause=function pause(){this.player.component.Pause();};SheetMusicController.prototype.previousBar=function previousBar(){};SheetMusicController.prototype.nextBar=function nextBar(){};angular.module('SheetMusic').controller('SheetMusicController',SheetMusicController);})();
(function(){'use strict';angular.module('SheetMusic').directive('sheetMusic',['$timeout',function($timeout){return{restrict:'E',templateUrl:assetDirectory+'/musictoolssongbook/js/SheetMusic/Partial/sheet-music.html',replace:true,scope:{file:'@'},bindToController:true,controller:'SheetMusicController',controllerAs:'sheetMusicCtrl',link:function(scope,element,attrs,sheetMusicCtrl){sheetMusicCtrl.component=$('.alphaTab');var $element=$(element);var sheetPosition=$element.offset();var sheetWidth=$element.width();var navbar=$element.find('.sheet-music-controls');navbar.offset({left:sheetPosition.left});navbar.width(sheetWidth);$timeout(function(){sheetMusicCtrl.component.alphaTab();sheetMusicCtrl.player.component=sheetMusicCtrl.component.alphaTab('playerInit',{asRoot:assetDirectory+'/musictoolssongbook/libraries/alphaTab/Samples/JavaScript/lib/alphaSynth/',swfObjectRoot:assetDirectory+'/musictoolssongbook/libraries/alphaTab/Samples/JavaScript/lib/alphaSynth/'});sheetMusicCtrl.player.component.On('ready',function(r){sheetMusicCtrl.player.component.LoadSoundFontUrl(assetDirectory+'/musictoolssongbook/libraries/alphaTab/Samples/JavaScript/lib/alphaSynth/default.sf2');});sheetMusicCtrl.player.component.On('soundFontLoad',function(loaded,full){var percentage=((loaded/full)*100)|0;$('#sfInfo .progress').text('('+percentage+'%)');});sheetMusicCtrl.player.component.On('soundFontLoaded',function(){$('#sfInfo').hide();});sheetMusicCtrl.player.component.On('readyForPlay',function(r){updateControls();scope.$apply();});sheetMusicCtrl.player.component.On('playerStateChanged',function(s){sheetMusicCtrl.player.state=s;updateControls();scope.$apply();});sheetMusicCtrl.component.alphaTab('playerCursor');},300);function updateControls(){if(!sheetMusicCtrl.player.ready){$('#loadingInfo').show()}
else{$('#loadingInfo').hide()}}}};}]);})();