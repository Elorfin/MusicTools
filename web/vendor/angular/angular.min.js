...
(function(){'use strict';angular.module('Note').factory('NoteService',['$http','$q',function($http,$q){var NoteService=function(){};NoteService.prototype={displayFlat:false,notes:[],current:null,getCurrent:function getCurrent(){return this.current;},setCurrent:function setCurrent(note){this.current=note;},setDisplayFlat:function setDisplayFlat(){},all:function all(){if(0!==this.notes.length){return this.notes}else{return this.find();}},find:function find(){var deferred=$q.defer();$http.get(Routing.generate('theory_note',{_format:'json'}),{}).success(function(response){this.notes=response;this.apply(function(note){if(this.displayFlat){}}.bind(this));deferred.resolve(response);}.bind(this)).error(function(response){deferred.reject(response);});return deferred.promise;},get:function get(value){return this.notes.find(function findByValue(element){return value==element.value;});},addSemitone:function addSemitone(reference,semitones){var newValue=(reference.value+semitones)%12;return this.get(newValue);},apply:function apply(callback){if(typeof callback==='function'){for(var i=0;i<this.notes.length;i++){callback(this.notes[i]);}}}};return new NoteService();}]);})();
...
(function(){'use strict';angular.module('Note').directive('noteSelector',['NoteService',function(NoteService){return{restrict:'E',templateUrl:assetDirectory+'/musictoolstheory/js/Note/Partial/selector.html',replace:true,scope:{},link:function(scope,element,attrs){NoteService.all().then(function(data){scope.notes=data;});}};}]);})();
